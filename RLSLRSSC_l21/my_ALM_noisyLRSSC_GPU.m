function [ C1, history ] = my_ALM_noisyLRSSC( A, B,lambda, lambda1,lambda2, ADAPTIVE_ON)%A,B
...
% min 0.5||B-AC||^2_F + lambda1||C||_* + lambda2||C||_1
...
ADAPTIVE_ON = gpuArray(ADAPTIVE_ON);
...
% optimal variables
rho0 = gpuArray(1.1);% this one can be tuned
rho=gpuArray(1);

epsilon=gpuArray(0.1);
eta=gpuArray(epsilon*5*n);
max_mu=gpuArray(1000);

tol_abs=gpuArray(1e-5);
tol_rel=gpuArray(1e-3);

max_iter=gpuArray(400);
iter=gpuArray(0);

%initialze
C1=zeros(k,n,'gpuArray');
C2=zeros(k,n,'gpuArray');
J=zeros(k,n,'gpuArray');
I_k=ones(k,1,'gpuArray');
I_n=ones(n,1,'gpuArray');
Lambda1=zeros(1,n,'gpuArray');
Lambda2=zeros(k,n,'gpuArray');
Lambda3=zeros(k,n,'gpuArray');
fun_val=gpuArray(0);
...
ATA=(A'*A); %
ATB=(A'*B);
invMat=pinv(lambda*ATA+mu1*ones(k,k,'gpuArray')+(mu2+mu3)*eye(k,'gpuArray'));% k*k

% diag_idx=logical(eye(n));%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fix me, delete?
converge=gpuArray(0);
while ~converge
    if rho~=1
        invMat=pinv(lambda*ATA+mu1*ones(k,k,'gpuArray')+(mu2+mu3)*eye(k,'gpuArray'));
    end
    

    J_new=invMat*(lambda*ATB+mu1*ones(k,n,'gpuArray')+mu2*C2+mu3*C1-I_k*Lambda1-Lambda2-Lambda3);%
